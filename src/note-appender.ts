...
export class NoteAppender {
	...
	private buildClaudeNotesSection(analysis: LLMAnalysis, grade: number): string {
	...
	private buildCorrectionsSection(corrections: CorrectionsResult, sectionNames: string[]): string {
		const lines: string[] = ['\n\n# Claude Review\n'];

		for (const section of sectionNames) {
			lines.push(`### ${section}`);
			const data = corrections.sections?.[section];
			const additions: string[] = Array.isArray(data?.additions) ? data.additions : [];
			const correctionsList: string[] = Array.isArray(data?.corrections) ? data.corrections : [];
