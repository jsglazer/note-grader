...
export class NoteGraderSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const compatUrl = new Setting(containerEl)
			.setName('Base URL')
			.setDesc(
				'API base URL. Ollama default: http://localhost:11434/v1  |  LM Studio: http://localhost:1234/v1',
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(`Minimum score to pass (currently: ${this.plugin.settings.gradeThreshold}).`)
			.addSlider((slider) =>
				slider
					.setLimits(0, 100, 1)
		...
		new Setting(containerEl)
			...
			.setDesc('Note-only uses your notes text only. PDF-assisted also reads the source PDF.')
			.addDropdown((drop) =>
				drop
		...
		const scriptSetting = new Setting(containerEl)
			...
			.setDesc('Absolute path to pdf_to_markdown.py. Required for PDF-assisted mode.')
			.addText((text) => {
				const checkScript = debounce(async (val: string) => {
					...
					setIndicator(
						...
						val ? (exists ? 'Script found ✓' : 'File not found ✗') : 'No path entered',
					);
				}, 500);
		...
		const venvToggle = new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.useVenv).onChange(async (value) => {
			...
			.addText((text) => {
				const checkVenv = debounce(async (val: string) => {
					...
					setIndicator(
						venvDot,
						val ? (exists ? 'valid' : 'invalid') : 'unchecked',
						val ? (exists ? `${python} found ✓` : `${python} not found ✗`) : 'No path entered',
					);
				}, 500);
		...
		new Setting(containerEl)
			.setName('Use OCR')
			.setDesc('Run Tesseract OCR before text extraction (needed for scanned PDFs; slower).')
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.useOcr).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc('Comma-separated list of section headings the plugin expects in your notes.')
			.addTextArea((area) =>
				area
